...
def environment(state, action, reward_map):
	...
	action_exe = -1
	if action == 1:
		if los < prob_back:  # 0.2 < 0.02
			action_exe = 3
		elif los < prob_back + prob_side:  # 0.2 < 0.02 + 0.16
			action_exe = 2
		elif los < prob_back + 2 * prob_side:  # 0.2 < 0.02 + 0.16 * 2
			action_exe = 4
		else:
			action_exe = 1
	elif action == 2:
...
def draw(reward_map, Q, file_name, reward=0):
	num_of_rows, num_of_columns = reward_map.shape
	...
	plt.title('Map: ' + file_name + " - best known strategy, avg r=" + str(reward))
	plt.show()
	f.savefig('image.svg')
